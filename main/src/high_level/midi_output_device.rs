...
impl MidiOutputDevice {
    ...
    pub fn get_name(&self) -> CString {
        let (_, name) = Reaper::get().medium.get_midi_output_name(self.id, 33);
        name.unwrap()
    }
    ...
    pub fn is_available(&self) -> bool {
        let (presence, name) = Reaper::get().medium.get_midi_output_name(self.id, 2);
    ...
    pub fn is_connected(&self) -> bool {
        ...
        Reaper::get().medium.get_midi_output_name(self.id, 1).0 == Presence::Present
    }
}
