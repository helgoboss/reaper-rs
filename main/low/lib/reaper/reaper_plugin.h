...
typedef struct _MIDI_eventprops {
  ...
  int varmsglen;
  int
      setflag; // &1:selected, &2:muted, &4:ppqpos, &8:endppqpos or bez tension, &16:msg1 high bits, &32:msg1 low bits, &64:msg2, &128:msg3, &256:varmsg, &1024:shape/tension fields used, &16384:no sort after set
} MIDI_eventprops;
...
typedef struct _REAPER_custom_action_register_t {
  int
      uniqueSectionId; // 0/100=main/main alt, 32063=media explorer, 32060=midi editor, 32061=midi event list editor, 32062=midi inline editor, etc
  const char*
      idStr; // must be unique across all sections for actions, NULL for reascripts (automatically generated)
...
typedef struct _REAPER_editor_register_t // register with "editor"
{
  int (* editFile)(const char* filename,
      HWND parent,
      int trackidx); // return TRUE if handled for this file
...
typedef struct _REAPER_project_import_register_t // register with "projectimport"
{
  ...
  const char* (* EnumFileExtensions)(int i,
      char** descptr); // call increasing i until returns NULL. if descptr's output is NULL, use last description
  int (* LoadProject)(const char* fn,
      ProjectStateContext* genstate); // return 0=ok, Generate RPP compatible project info in genstate
...
typedef struct project_config_extension_t // register with "projectconfig"
{
  ...
  bool (* ProcessExtensionLine)(const char* line,
      ProjectStateContext* ctx,
      bool isUndo,
      struct project_config_extension_t* reg); // returns BOOL if line (and optionally subsequent lines) processed (return false if not plug-ins line)
  void (* SaveExtensionConfig)(ProjectStateContext* ctx,
      bool isUndo,
...
typedef struct audio_hook_register_t {
  void (* OnAudioBuffer)(bool isPost,
      int len,
      double srate,
      struct audio_hook_register_t* reg); // called twice per frame, isPost being false then true
  void* userdata1;
  void* userdata2;
...
*/
enum {
  SCREENSET_ACTION_GETHWND =
  0, // returns HWND of screenset window. If searching for a specific window, it will be passed in actionParm

  SCREENSET_ACTION_IS_DOCKED = 1, // returns 1 if docked
  SCREENSET_ACTION_SWITCH_DOCK = 4, //dock if undocked and vice-versa

  SCREENSET_ACTION_LOAD_STATE =
  0x100, // load state from actionParm (of actionParmSize). if both are NULL, hide.
...
typedef LRESULT(* screensetNewCallbackFunc)
    (int action, const char* id, void* param, void* actionParm, int actionParmSize);
