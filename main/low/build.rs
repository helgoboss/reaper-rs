...
mod codegen {
    ...
    mod phase_two {
        ...
        fn generate_swell_token_stream(fn_ptrs: &Vec<FnPtr>) -> proc_macro2::TokenStream {
            ...
            let functions: Vec<_> = fn_ptrs.iter().map(|p| generate_function(&p)).collect();
            let windows_methods: Vec<_> = fn_ptrs
                .iter()
                .map(|p| generate_method(&p, generate_swell_windows_method_body(&p)))
                .collect();
            quote::quote! {
                //! This file is automatically generated by executing `cargo build --features generate`.
                //!
        ...
        fn generate_method(ptr: &FnPtr, body: Block) -> ImplItem {
            ...
            ImplItem::Method(ImplItemMethod {
                ...
                defaultness: None,
                sig: extract_signature(ptr, has_pointer_args, true),
                block: body,
            })
        }

        /// Generates a "extern C" free function definition
        fn generate_function(ptr: &FnPtr) -> Item {
            Item::ForeignMod(ItemForeignMod {
                ...
                items: vec![ForeignItem::Fn(ForeignItemFn {
                    ...
                    sig: extract_signature(ptr, false, false),
        ...
        fn extract_signature(ptr: &FnPtr, make_unsafe: bool, as_method: bool) -> Signature {
            Signature {
                constness: None,
                asyncness: None,
        ...
        fn generate_swell_windows_method_body(ptr: &FnPtr) -> Block {
            let fn_ptr_call = generate_fn_ptr_call(&ptr.signature, ptr.name.clone());
            syn::parse_quote! {
                {
